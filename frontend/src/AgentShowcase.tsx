...
import type { AgentMessage, AgentToolCall, FulfillmentStatus } from "./types";
import { api } from "./api";
...
const labelForTool: Record<string, string> = {
  get_order: "查询订单",
  get_logistics: "查询物流",
  check_after_sales_eligibility: "校验售后资格",
  create_after_sales_case: "创建待确认售后单",
  confirm_after_sales_case: "确认售后单",
  cancel_after_sales_case: "取消售后单",
  schedule_pickup: "预约取件",
  get_fulfillment_status: "查询履约进度",
  retrieve_knowledge: "检索已发布知识",
  create_review_ticket: "提交人工审核",
};
...
function newThreadId() {
  return `customer-web-${crypto.randomUUID()}`;
}
...
export function AgentShowcase() {
  ...
  const activeItem = useMemo(
    () => [...items].reverse().find((item) => item.role === "agent" && item.result?.run_id === activeRunId),
    [items, activeRunId],
  );
  ...
  async function send() {
    ...
    try {
      ...
      messageApi.error(error instanceof Error ? error.message : "Agent 请求失败，请稍后重试。");
    } finally {
      setSending(false);
    }
  }
  ...
  async function resolveConfirmation(item: ChatItem, approved: boolean) {
    ...
    try {
      ...
      messageApi.error(error instanceof Error ? error.message : "确认失败，请重试。");
    } finally {
      setConfirming(false);
    }
  }
  ...
  return <>{contextHolder}<Layout className="agent-shell">
    <Layout.Header className="agent-header">
      ...
      <Space><Select value={actor} onChange={(value) => { setActor(value); resetConversation(); }} options={[
        { value: "U001", label: "演示客户 · 李雷" },
        { value: "U7001", label: "M7 演示客户" },
      ]} /><Button onClick={resetConversation}>新建会话</Button><a href="/">运营台</a></Space>
    </Layout.Header>
    <Layout.Content className="agent-content">
      <section className="agent-hero">
        <div><Typography.Title level={1}>售后问题，给你可核验的答案</Typography.Title><Typography.Paragraph>Agent 只能调用受控工具。金额、资格、权限和履约状态都由后端服务与数据库决定。</Typography.Paragraph></div>
        <Space wrap><Tag color="blue">LangGraph</Tag><Tag color="purple">DeepSeek v4 Flash</Tag><Tag color="green">确认后执行</Tag><Tag color="gold">全程 Trace</Tag></Space>
      </section>
      <Row gutter={[20, 20]}>
        <Col xs={24} lg={15}>
          <Card className="agent-chat-card" title={<Space><Avatar style={{ background: "#1677ff" }}>V</Avatar><span>VeriReturn 售后助手</span><Tag color="green">在线</Tag></Space>} extra={<Typography.Text type="secondary">会话 {threadId.slice(-8)}</Typography.Text>}>
            {items.length === 0 ? <div className="agent-empty"><Empty description="从一个售后问题开始" /><Space wrap>{suggestedPrompts.map((prompt) => <Button key={prompt} onClick={() => setDraft(prompt)}>{prompt}</Button>)}</Space></div> : <List className="agent-messages" dataSource={items} renderItem={(item) => <List.Item className={`agent-message agent-message-${item.role}`}>
            ...
            {sending && <div className="agent-thinking"><Spin size="small" /> 正在解析意图并调用受控工具…</div>}
            <Divider />
            <Input.TextArea value={draft} onChange={(event) => setDraft(event.target.value)} onPressEnter={(event) => { if (!event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="例如：帮我退 O1001，商品未拆封" autoSize={{ minRows: 3, maxRows: 6 }} />
            <div className="agent-compose-actions"><Typography.Text type="secondary">Enter 发送，Shift + Enter 换行</Typography.Text><Button type="primary" loading={sending} onClick={() => void send()}>发送</Button></div>
          </Card>
        </Col>
        <Col xs={24} lg={9}>
          <Space direction="vertical" size="middle" className="agent-side-stack">
            <Card title="本次受控执行" extra={activeItem?.result && resultTag(activeItem.result.status)}>
              {activeItem ? <>
                <Descriptions size="small" column={1}><Descriptions.Item label="Run"><Typography.Text copyable>{activeItem.result?.run_id}</Typography.Text></Descriptions.Item><Descriptions.Item label="业务对象">{activeItem.result?.case_id ? `售后单 #${activeItem.result.case_id}` : activeItem.result?.ticket_id ? `审核单 #${activeItem.result.ticket_id.slice(0, 8)}` : "仅查询"}</Descriptions.Item></Descriptions>
                <Steps direction="vertical" size="small" current={activeItem.tools?.length || 0} items={(activeItem.tools || []).map((tool) => ({ title: labelForTool[tool.tool_name] || tool.tool_name, description: <Space><Tag color={tool.status === "succeeded" ? "green" : "red"}>{tool.status}</Tag><span>{tool.latency_ms} ms</span><Tooltip title={JSON.stringify(tool.arguments_json)}><Typography.Text type="secondary">参数</Typography.Text></Tooltip></Space>, status: tool.status === "succeeded" ? "finish" : "error" }))} />
              </> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="发送消息后显示工具 Trace" />}
            </Card>
            <Card title="为什么值得信任？"><Space direction="vertical"><Typography.Text>✓ Agent 不能直连数据库</Typography.Text><Typography.Text>✓ 高风险写操作必须确认</Typography.Text><Typography.Text>✓ 只检索已发布的知识版本</Typography.Text><Typography.Text>✓ 履约只接受验签 Provider 事实</Typography.Text><Typography.Text>✓ 每次调用都可关联审计与 Trace</Typography.Text></Space></Card>
